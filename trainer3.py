...
class Trainer3(Trainer):
    def build_model(self):
        ...
        print('self.use_c: %s' % self.use_c)
        print('self.output_shape: %s' % self.output_shape)
    ...
    def train_(self):
        ...
        print('len(z_samples): %s' % len(z_samples))
        print('len(z_samples[0]): %s' % len(z_samples[0]))
    ...
    def test_(self):
        ...
        #     G_s, Gnorm = self.sess.run([self.G_s, self.G], {self.z: z_c[self.test_b_num*b:self.test_b_num*(b+1),:]})
        ...
        print('z_c.shape[0]/5: %s' % (z_c.shape[0]/5))
        for i in range(int(z_c.shape[0]/5)):
            self.generateTestImg([z_c[5*i:5*i+5,:]], root_path=out_dir, idx=i)
    ...
    def build_test_model(self):
        ...
        # for debug
        self.G = denorm_img3(self.G_)
        self.G_jaco_, self.G_vort_ = jacobian3(self.G_)
        self.G_vort = denorm_img3(self.G_vort_)


    def generateTestImg(self, inputs, root_path=None, idx=None):
        # xy_list = []
        # zy_list = []
        xym_list = []
        zym_list = []
        ...
        for _, z_sample in enumerate(inputs):
            ...
            print(z_sample.shape)
            print('z_sample.dtype:')
            print(z_sample.dtype)

            xym, zym, xymw, zymw = self.sess.run( # xy, zy,
                [self.G['xym'], self.G['zym'], self.G_vort['xym'], self.G_vort['zym']], {self.z: z_sample}) # self.G['xy'], self.G['zy'],
            # xy_list.append(xy)
            # zy_list.append(zy)
            # xyw_list.append(xy)
            # zyw_list.append(zy)

            print('xym.shape, zym.shape:')
            print(xym.shape)
            print(zym.shape)
            print('xymw.shape, zymw.shape:')
            print(xymw.shape)
            print(zymw.shape)

            xym_list.append(xym)
            zym_list.append(zym)
            xymw_list.append(xymw)
            zymw_list.append(zymw)
        ...
        for tag, generated in zip(['xym','zym'], # '0_xy','0_zy',
            ...
            c_concat = np.concatenate(tuple(generated), axis=0)
        ...
        for dim in gen_random.shape:
            print('gen_random.shape: %s' % dim)
